...
export default function RegisterPage() {
  ...
  return (
    <div className="min-h-[calc(100vh-3.5rem)] flex items-center justify-center bg-gradient-to-br from-primary-50 to-purple-50 px-4">
      <div className="w-full max-w-md bg-white rounded-3xl shadow-xl p-8 animate-slide-up">
        <div className="text-center mb-8">
          ...
          <p className="text-sm text-gray-500 mt-1">加入 PersonaQuest 开始探索</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            <div className="grid grid-cols-3 gap-2">
              {(['STUDENT', 'CREATOR', 'ADMIN'] as Role[]).map((r) => (
                <button
                  key={r}
                  type="button"
                  onClick={() => setRole(r)}
                ...
                >
                  {r === 'STUDENT' ? '学生' : r === 'CREATOR' ? '运营者' : '管理员'}
                </button>
              ))}
            </div>
          </div>
